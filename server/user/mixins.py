...
class UserRelationshipDataHandlerMixin(SessionDecoderMixin):
    def get_response_data(self, frontend_session_key, relationship_type, profile_user_id):
        ...
        serializerObj = self.get_serializer_obj(login_user_id, id_list)
    ...
    def get_serializer_obj(self, login_user_id, id_list):
        user = User.objects.filter(id__in=id_list)

        return UserRelationshipSerializer(user, many = True, context={'login_user_id' : login_user_id})