...
class ChatRoomSenderSerializer(serializers.ModelSerializer):
    target = serializers.IntegerField(source = 'sender.id', read_only=True)
    target_name = serializers.CharField(source = 'sender.name', read_only=True)
    target_first_campus_name = serializers.CharField(source = 'sender.first_course.campus.name', read_only=True)
...
class ChatRoomReceiverSerializer(serializers.ModelSerializer):
    target = serializers.IntegerField(source = 'receiver.id', read_only=True)
    target_name = serializers.CharField(source = 'receiver.name', read_only=True)
    target_first_campus_name = serializers.CharField(source = 'receiver.first_course.campus.name', read_only=True)
    target_second_campus_name = serializers.SerializerMethodField()
    profile_img_path = serializers.SerializerMethodField()
    latest_date = serializers.DateTimeField(format = "%Y년 %m월 %d일")
    ...
    def get_target_second_campus_name(self, chatroom):
        try:
            sender_second_course_campus_name = chatroom.receiver.second_course.campus.name
        except:
            return ""
    ...
    def get_profile_img_path(self, chatroom):
        profile = Profile.objects.get(user = chatroom.receiver.id)
        if profile.img_path:
            profile_img_path = profile.img_path
        else:
            profile_img_path = '/media/profile/default_profile.png'
